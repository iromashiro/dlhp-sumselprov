...
<section class="bg-white reveal">
    <div class="w-full px-5 py-6 mx-auto space-y-5 sm:py-8 md:py-12 sm:space-y-8 md:space-y-16 max-w-7xl">
        ...
        <!-- BERITA HEADER -->
        @foreach ($berita_header as $bh)
        <div class="flex flex-col items-center sm:px-5 md:flex-row">
            <div class="w-full md:w-1/2">
                <a href="{{ route('berita.single', $bh->slug) }}" class="block">
                    <img class="object-cover w-full h-full rounded-lg max-h-64 sm:max-h-96"
                        src="{{ URL::asset($bh->thumbnail) }}">
                </a>
            </div>
            <div class="flex flex-col items-start justify-center w-full h-full py-6 mb-6 md:mb-0 md:w-1/2">
                <div
                    ...
                    <h1 class="text-4xl font-bold leading-none lg:text-5xl xl:text-6xl"><a
                            href="{{ route('berita.single', $bh->slug) }}">{!!
                            Str::limit($bh->judul, 30) !!}</a></a></h1>
                    <p class="pt-2 text-sm font-medium">by <a href="#_" class="mr-1 underline">admin</a> · <span
                            class="mx-1">Tanggal : {{ $bh->created_at->format('d F Y') }}</span>
                    </p>
                </div>
            </div>
        </div>
        ...
        <div class="flex grid grid-cols-12 pb-10 sm:px-5 gap-x-8 gap-y-16">
            ...
            @foreach ($berita as $b)
            <div class="flex flex-col items-start col-span-12 space-y-3 sm:col-span-6 xl:col-span-4 {{ $b->slug }}">
                <a href="{{ route('berita.single', $b->slug) }}" class="block">
                ...
                <h2 class="text-lg font-bold sm:text-xl md:text-2xl"><a
                        href="{{ route('berita.single', $b->slug) }}">{!!
                        Str::limit($b->judul, 20) !!}</a>
                </h2>
                <p class="text-sm text-gray-500">{!!
                    Str::limit($b->isi_berita, 30) !!}</p>
                <p class="pt-2 text-xs font-medium"><a href="#_" class="mr-1 underline">Admin</a> · <span
                        class="mx-1">Tanggal : {{ $bh->created_at->format('d F Y') }}</span></p>
            </div>
            @endforeach
